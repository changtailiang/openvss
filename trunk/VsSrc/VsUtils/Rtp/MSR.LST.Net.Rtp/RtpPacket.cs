...
namespace Vs.Provider.RtpLib.MSR.LST.Net.Rtp
{
    ...
    internal class RtpPacket : RtpPacketBase
    {
        ...
        private const int HEADER_EXTENSIONS_SIZE = PACKETS_IN_FRAME_SIZE +
                                                   FRAME_INDEX_SIZE +
                                                   FEC_INDEX_SIZE;
        private const int HEADER_EXTENSIONS_DATA_SIZE = HEADER_EXTENSIONS_SIZE +
                                                   JPG_HAD_SIZE;
        ...
        ///  JPEG header
        /// </summary>
        private const int JPG_HAD_SIZE = 8;
        private const int JPG_SPC_SIZE = 1;
        private const int JPG_OFF_SIZE = 3;
        private const int JPG_TYP_SIZE = 1;
        private const int JPG_QUL_SIZE = 1;
        private const int JPG_WID_SIZE = 1;
        private const int JPG_HIG_SIZE = 1;

        private const int JPG_SPC_INDEX = 0;
        private const int JPG_OFF_INDEX = JPG_SPC_INDEX + JPG_SPC_SIZE;
        private const int JPG_TYP_INDEX = JPG_OFF_INDEX + JPG_OFF_SIZE;
        private const int JPG_QUL_INDEX = JPG_TYP_INDEX + JPG_TYP_SIZE;
        private const int JPG_WID_INDEX = JPG_QUL_INDEX + JPG_QUL_SIZE;
        private const int JPG_HIG_INDEX = JPG_WID_INDEX + JPG_WID_SIZE;
        ...
        {
            get{return Buffer.GetUInt16(FrameIndex_Index);}
        ...
        {
            get{return (base.HeaderSize + JPG_HAD_SIZE);}
        }

        internal int JpgHeaderIndex
        {
            get{return base.HeaderSize;}
        }

        internal int JpgSpec
        {
            get{return (int)(Buffer[JpgHeaderIndex]);}
            set{Buffer[JpgHeaderIndex] = (byte)value;}
        }

        internal int JpgOffset
        {
            get{return (int)(Buffer.GetInt32(JpgHeaderIndex + JPG_OFF_INDEX) >> 8);}
            set{Buffer.SetInt32(JpgHeaderIndex + JPG_OFF_INDEX, (value << 8));}
        }

        internal int JpgType
        {
            get{return (int)(Buffer[JpgHeaderIndex + JPG_TYP_INDEX]);}
            set{Buffer[JpgHeaderIndex + JPG_TYP_INDEX] = (byte)value;}
        }

        internal int JpgQuality
        {
            get{return (int)(Buffer[JpgHeaderIndex + JPG_QUL_INDEX]);}
            set{Buffer[JpgHeaderIndex + JPG_QUL_INDEX] = (byte)value;}
        }

        internal int JpgWidth
        {
            get{return 8*(int)(Buffer[JpgHeaderIndex + JPG_WID_INDEX]);}
            set{Buffer[JpgHeaderIndex + JPG_WID_INDEX] = (byte)(value/8);}
        }

        internal int JpgHeight
        {
            get{return 8*(int)(Buffer[JpgHeaderIndex + JPG_HIG_INDEX]);}
            set{Buffer[JpgHeaderIndex + JPG_HIG_INDEX] = (byte)(value/8);}
        }
