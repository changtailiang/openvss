...
//		new void OpenStream([In, MarshalAs(UnmanagedType.Interface)] IStream pStream);
	...
	{
		void OnStreamSample([In] ushort wStreamNum,
					  [In] ulong cnsSampleTime,
					  [In] ulong cnsSampleDuration,
					  [In] uint dwFlags,
					  [In, MarshalAs(UnmanagedType.Interface)] INSSBuffer pSample,
		...
		void OnTime([In] ulong cnsCurrentTime,
					  [In] IntPtr pvContext);

		void OnStreamSelection([In] ushort wStreamCount,
					  [Out] out ushort pStreamNumbers,
					  [In] ref WMT_STREAM_SELECTION pSelections,
					  [In] IntPtr pvContext);

		void OnOutputPropsChanged([In] uint dwOutputNum,
					  [In] ref WM_MEDIA_TYPE pMediaType,
					  [In] IntPtr pvContext);

		void AllocateForStream([In] ushort wStreamNum,
					[In] uint cbBuffer,
					[Out] out INSSBuffer ppBuffer,
					[In] IntPtr pvContext);

		void AllocateForOutput([In] uint dwOutputNum,
					[In] uint cbBuffer,
					[Out] out INSSBuffer ppBuffer,
					[In] IntPtr pvContext);
	}
