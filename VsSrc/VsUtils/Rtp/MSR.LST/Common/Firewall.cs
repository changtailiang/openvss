...
namespace Vs.Provider.RtpLib.MSR.LST.Net
{
    /// <remarks>
    ...
    public abstract class FirewallUtility
    {
        ...
        public static void AddFirewallException (string exceptionName, ushort[] ports, ProtocolType[] protocols, string appPath)
        {
            ...
            else
            {
                if (ports.Length != protocols.Length)
                    throw new ArgumentException("The arrays of ports and port types should be the same length.");
        ...
        private static void ValidateForOldCompatibleFirewall()
        {
            ...
            if (os.Version.Major < 5) // earlier than Win2K
                throw new NotSupportedException("For a firewall port map to be added, the OS must be Windows XP SP2 or greater.");
            ...
            if( HasSp2Firewall ) // Skip computers with firewall == SP2's firewall (i.e. NOT XP pre-SP2 & Win2K3)
                throw new NotSupportedException("Do not call AddPortToOldFirewall or RemovePortFromOldFirewall on a comptuer with an XP SP2-compatible firewall.");
        }
        ...
        private static void ValidateAdministrator()
        {
            ...
            if (wp.IsInRole(WindowsBuiltInRole.Administrator) == false)
                throw new InvalidOperationException("You must be an administrator to modify the firewall settings.");
        }
        ...
        private static byte ConvertAndValidateProtocol(ProtocolType protocol)
        {
            switch(protocol)
            {
                ...
                default:
                    throw new NotSupportedException("That protocol is not supported by this firewall wrapper.  Only TCP & UDP are supported.");
            }
        }
        #endregion
